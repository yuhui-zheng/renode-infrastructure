...
namespace Antmicro.Renode.Utilities.Packets
{
    public class Packet
    {
        ...
        {
            return cache.Get(typeof(T), x => CalculateLengthInner<T>());
        }

        public static int CalculateOffset<T>(string fieldName)
        {
            return cache.Get(Tuple.Create(typeof(T), fieldName), x => CalculateOffsetInner<T>(fieldName));
        }
        ...
        public static T Decode<T>(IList<byte> data, int dataOffset = 0)
        {
            ...
            foreach(var field in fieldsAndProperties)
            {
                ...
                else if(type == typeof(byte[]))
                {
                    ...
                    if(width == 0)
                    {
                        throw new ArgumentException("Width must be provided to decode byte array");
                    }
        ...
        private static int CalculateLengthInner<T>()
        {
            var fieldsAndProperties = cache.Get(typeof(T), x => GetFieldsAndProperties<T>());
        ...
        private static int CalculateOffsetInner<T>(string fieldName)
        {
            var fieldsAndProperties = cache.Get(typeof(T), x => GetFieldsAndProperties<T>());
